...
- (void)commonInit {
    ...
    self.frame = CGRectMake(0, 0, KWidth, 260 * HORIZENTAL_SCALE);
    
    // Set visible viewport to one contains today by default.
    NSDate *todayDate = [NSDate dateWithTimeIntervalSinceNow:0];
    NSDateComponents *comps = [CalendarAssistant dateComponentsFromDate:todayDate];
    ...
    self.navigationBar = [[DAYNavigationBar alloc] initWithFrame:CGRectMake(0, 0, self.layout_width, 44)];
    ...
    self.weekHeaderView.layout_size = CGSizeMake(self.layout_width, (self.layout_height - self.navigationBar.layout_y - self.indicatorRadius)/7);
    [self.weekHeaderView changeAnchorPoint:0 :0 toPoint:0 :self.navigationBar.layout_max_y];
    
    self.contentWrapperView = [[UIView alloc] init];
    [self addSubview:self.contentWrapperView];
    self.contentWrapperView.layout_size = CGSizeMake(self.layout_width, (self.layout_height - self.navigationBar.layout_y - self.indicatorRadius)/7 * 6);
    [self.contentWrapperView changeAnchorPoint:0 :0 toPoint:0 :self.weekHeaderView.layout_max_y];
...
- (void)setSelectedDate:(NSDate *)selectedDate {
    NSDateComponents *comps = [CalendarAssistant dateComponentsFromDate:selectedDate];
    int64_t delayTime = 0;
...
- (void)configureWeekdayHeaderView {
    ...
    [self.weekHeaderView.arrangedSubviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (canUseLocalizedStrings) {
            ...
            weekdayLabel.text = [CalendarAssistant stringOfWeekdayInEnglish:idx + 1];
        }
    }];
}
...
- (void)configureComponentView:(DAYComponentView *)view withDay:(NSUInteger)day month:(NSUInteger)month year:(NSUInteger)year {
    ...
    if ([CalendarAssistant isDateTodayWithDateComponents:comps]) {
        ...
        if (self->_currentVisibleRow == 0) {
            NSUInteger paddingDays = [CalendarAssistant firstWeekdayInMonth:self->_visibleMonth ofYear:self->_visibleYear] - 1;
    ...
    if (self.showUserEvents) {
        [self->_eventsInVisibleMonth enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            EKEvent *event = obj;
            if ([[CalendarAssistant dateFromComponents:comps] isEqualToDate:event.startDate]) {
...
- (void)configureContentView {
    ...
    NSUInteger totalDays = [CalendarAssistant daysInMonth:self->_visibleMonth ofYear:self->_visibleYear];
    NSUInteger paddingDays = [CalendarAssistant firstWeekdayInMonth:self->_visibleMonth ofYear:self->_visibleYear] - 1;
    ...
    if (self.showUserEvents) {
        ...
        NSPredicate *predicate = [self.eventStore predicateForEventsWithStartDate:[CalendarAssistant dateFromComponents:startComps]
                                                                          endDate:[CalendarAssistant dateFromComponents:endComps]
                                                                        calendars:nil];
    ...
    NSUInteger totalDaysInLastMonth = [CalendarAssistant daysInMonth:paddingMonth ofYear:paddingYear];
...
//    NSString *stringOfMonth = [CalendarAssistant stringOfMonthInEnglish:self->_visibleMonth];
    return [NSString stringWithFormat:@"%lu年%lu月",(unsigned long) self->_visibleYear,(unsigned long)_visibleMonth];
}
...
- (void)componentDidTap:(DAYComponentView *)sender {
    ...
    self->_selectedDate = [CalendarAssistant dateFromComponents:comps];
...
- (void)jumpToPreviousMonth {
    if (self.singleRowMode) {
        if (self->_currentVisibleRow > 0) {
            ...
            // by Rakuyo. Optimize jump logic
            NSUInteger totalDays = [CalendarAssistant daysInMonth:self->_visibleMonth - 1 ofYear:self->_visibleYear];
            NSUInteger paddingDays = [CalendarAssistant firstWeekdayInMonth:self->_visibleMonth - 1  ofYear:self->_visibleYear] - 1;
